...
class TorrentFilesController: ThemedUIViewController {
    ...
    override func viewDidLoad() {
        ...
        if path.pathComponents.count > 1 {
            ...
            NotificationCenter.default.addObserver(self, selector: #selector(updateData), name: .torrentsUpdated, object: nil)
        }
    ...
    func setFilesPriority() {
        TorrentSdk.setTorrentFilesPriority(hash: torrentHash, states: files.map { $0.priority.rawValue })
    }
